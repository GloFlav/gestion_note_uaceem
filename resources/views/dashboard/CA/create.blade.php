...
@section('content')
 ...
 <div class=" px-1 py-1 sm:px-6 lg:px-1 mx-auto"><!-- Card -->
    <div class=" p-4 sm:p-1 ">
        ...
        <form method="POST" action="{{ route('dashboard.candidataceem.store', $candidats->id) }}">
            @csrf
        ...
        <!-- End Section -->
          ...
          <div class="mt-5 flex justify-end gap-x-2">
            <a href="{{ route('dashboard.candidataceem.index')}}" type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
              Annuler
            </a>
...
<script>
    ...
    selectMention.addEventListener('change', function() {
        if (this.value === 'concours') {
            montantInput.value = formatNumber(0); // Remplir le montant avec 0 000
        }
    });
