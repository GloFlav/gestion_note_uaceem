...
@section('content')
        ...
        <div class=" p-4 sm:p-1 ">
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-600 dark:text-neutral-400">
                    Pour modifier les informations d'un niveau dans le système, veuillez remplir le formulaire
            ...
            <form method="POST">
                ...
                <div class="relative">
                    ...
                    <select
                        data-hs-select='{
    "placeholder": "Selectionner Mention...",
    ...
    "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"shrink-0 size-3.5 text-gray-500 dark:text-neutral-500 \" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
                        ...
                        class="hidden" name="parcours_id">
                        @php
                            $parcoursValue = old('parcours_id') ?? $niveau->parcours_id;
                        @endphp
                        @foreach ($parcours as $parcour)
                            <option value="{{ $parcour->id }}" {{ $parcoursValue == $parcour->id ? 'selected' : '' }}>
                                {{ $parcour->design }} - {{ $parcour->mention->code }}
                            </option>
                        @endforeach
                    </select>
                    <!-- End Select -->
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative ">
                        <textarea id="descriotion"
                            ...
                            placeholder="Description du niveau" name="description">{{ old('description') }}</textarea>
                        @error('description')
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const allLevels = ['L1', 'L2', 'L3', 'M1', 'M2'];
            const addedLevels = @json($niveauxTaken);
            const currentNiveau = "{{ $niveau->design }}";
            const currentParcours = {{ $niveau->parcours_id }}
            const parcoursSelect = document.querySelector('select[name="parcours_id"]');
            const niveauSelect = document.querySelector('select[name="design"]');

            const updateNiveau = function() {
                const selectedParcoursId = +parcoursSelect.value;
                const addedLevelsForParcours = addedLevels[selectedParcoursId] || [];

                // Clear existing options
                niveauSelect.innerHTML = '';

                // Add available levels and the current niveau
                allLevels.forEach(level => {
                    if (!addedLevelsForParcours.includes(level) ||
                        selectedParcoursId === currentParcours &&
                        level === currentNiveau) {
                        const option = document.createElement('option');
                        option.value = level;
                        option.textContent = level;
                        if (selectedParcoursId === currentParcours &&
                            level === currentNiveau) {
            ...
            parcoursSelect.addEventListener('change', updateNiveau);
            updateNiveau();
        })
    </script>
@endsection
