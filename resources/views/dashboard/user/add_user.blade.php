        ...
        <div class=" p-4 sm:p-1 ">
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-600 dark:text-neutral-400">
                    Pour ajouter un nouveau utilisateur au système, veuillez remplir le formulaire ci-dessous avec les
            ...
            <form method="POST">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input id="username" type="text"
                            ...
                            shadow-sm text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                            placeholder="Nom d'utilisateur" name="username" value="{{ old('username') }}">
                        @error('username')
                            <div class="absolute inset-y-0 end-0 flex items-center pointer-events-none pe-3">
                                <svg class="shrink-0 size-4 text-red-500" xmlns="http://www.w3.org/2000/svg" width="24"
                                    height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                ...
                <div class="space-y-2">
                    <label for="password"
                        class="inline-block text-sm font-medium text-gray-800 mt-2.5 dark:text-neutral-200">
                        Mot de passe (par défaut) <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select id="role"
                            ...
                            shadow-sm text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                            name="role">
                            <option value="SI Admin" {{ old('role') == 'SI Admin' ? 'selected' : '' }}>SI Admin</option>
                            <option value="Caisse" {{ old('role') == 'Caisse' ? 'selected' : '' }}>Caissier</option>
                            <option value="CA" {{ old('role') == 'CA' ? 'selected' : '' }}>Caisse Candidat ACEEM(CA)
                            </option>
                            <option value="Accueil" {{ old('role') == 'Accueil' ? 'selected' : '' }}>Accueil</option>
                            <option value="Agent" {{ old('role') == 'Agent' ? 'selected' : '' }}>Agent</option>
                            <option value="Scolarité" {{ old('role') == 'Scolarité' ? 'selected' : '' }}>Scolarité</option>
                            <option value="Etudiant" {{ old('role') == 'Etudiant' ? 'selected' : '' }}>Etudiant</option>

                            <option value="Inscription" {{ old('role') == 'Inscription' ? 'selected' : '' }}>Inscription</option>
                            <option value="Recteur" {{ old('role') == 'Recteur' ? 'selected' : '' }}>Recteur</option>
                            <option value="Responsable Note" {{ old('role') == 'Responsable Note' ? 'selected' : '' }}>Responsable Note</option>
                            <option value="Caisse evenement" {{ old('role') == 'Caisse evenement' ? 'selected' : '' }}>
                                Caisse Evenement
                            </option>
                        </select>
                        @error('role')
