                        ...
                        <div class="py-4 grid gap-3 md:flex flex-col  border-b border-gray-200 dark:border-neutral-700">
                            <div>
                                <h2 class="text-2xl font-semibold text-gray-800 dark:text-neutral-200 mb-4">
                                    Gestion de Réinscription
                                </h2>
                                <p class="text-sm text-gray-600 dark:text-neutral-400">
                                    Gérez la partie de réinscription en recherchant les étudiants, pour la continuation de
                                    ces études aux divers niveaux et bien plus. <br>
                                    Bien effectuer la réinscription, ses types et modes.
                                </p>
                            </div>
                                    ...
                                    <form class="relative">
                                        ...
                                        @if ($key != '')
                                            <div class=" absolute inset-y-0 end-0 flex items-center z-20 pe-1">
                                                <a href="{{ route('dashboard.reinscription.index') }}"
                                ...
                                @foreach ($etudiants as $etudiant)
                                    <tr>
                                        <td
                                            ...
                                            @if ($etudiant->payement && !empty($etudiant->payement->reference))
                                                <a class="inline-flex items-center gap-x-1 text-sm text-gray-600 decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-gray-500"
                                                    href="{{ route('dashboard.reinscription.show', $etudiant->id) }}">
                                                    {{ $etudiant->nom }}
                                                </a>
                                            @else
                                                <a class="inline-flex items-center gap-x-1 text-sm text-gray-600 decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-gray-500"
                                                    href="{{ route('dashboard.reinscription.create', $etudiant->id) }}">
                                                    {{ $etudiant->nom }}
                                                </a>
                                            @endif
                                        </td>
                                        ...
                                        <td class="h-px whitespace-nowrap px-6">
                                            @if ($etudiant->payement && !empty($etudiant->payement->reference))
                                                <a class="inline-flex items-center gap-x-1 text-sm text-gray-600 decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-gray-500"
                                                    href="{{ route('dashboard.reinscription.show', $etudiant->id) }}">
                                                    @if ($etudiant->matriculeInfo)
                                                        {{ $etudiant->matriculeInfo->design }}
                                                    @else
                                                        <p>Pas de matricule disponible</p>
                                                    @endif
                                                </a>
                                            @else
                                                <a class="inline-flex items-center gap-x-1 text-sm text-gray-600 decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-gray-500"
                        ...
                        <div
                            ...
                            <div>
                                <div class="inline-flex gap-x-2">
                                    <a href="{{ request()->fullUrlWithQuery(array_merge(request()->all(), ['page' => $etudiants->currentPage() - 1 > 0 ? $etudiants->currentPage() - 1 : ''])) }}"
                                        class="py-1.5 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm cursor-pointer hover:bg-gray-50 disabled:opacity-50  disabled:cursor-default disabled:pointer-events-none focus:outline-none focus:bg-gray-50 dark:bg-transparent dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800
                                         @if ($etudiants->currentPage() == 1) pointer-events-none cursor-default opacity-50 @endif">
                                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24"
                                    ...
                                    <a href="{{ request()->fullUrlWithQuery(array_merge(request()->all(), ['page' => $etudiants->currentPage() + 1])) }}"
                                        class="py-1.5 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm cursor-pointer hover:bg-gray-50 disabled:opacity-50 disabled:cursor-default disabled:pointer-events-none focus:outline-none focus:bg-gray-50 dark:bg-transparent dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800
                                         @if ($etudiants->currentPage() == $etudiants->lastPage()) pointer-events-none cursor-default opacity-50 @endif">
                                        Suiv.
                                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24"
                                            height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
