...
@section('content')
        ...
        <div class=" p-4 sm:p-1 ">
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-600 dark:text-neutral-400">
                    Pour modifier les informations d'un parcours dans le système, veuillez remplir le formulaire
            ...
            <form method="POST" action="{{ route('dashboard.academique.parcours.update', ['id' => $parcours->id]) }}">
                @method('put')
                @csrf
                <div class="space-y-2">
                    <label for="design"
                    ...
                    <div class="relative">
                        <input id="design" type="text"
                            ...
                            placeholder="Désignation" name="design" value="{{ old('design') ?? $parcours->design }}">
                        @error('design')
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <textarea id="design"
                            ...
                            shadow-sm text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                            placeholder="Description de la Mention" name="description" value="{{ old('description') }}">{{ old('description') ?? $parcours->description }}</textarea>
                        @error('description')
                            <div class="absolute inset-y-0 end-0 flex items-center pointer-events-none pe-3">
                                <svg class="shrink-0 size-4 text-red-500" xmlns="http://www.w3.org/2000/svg" width="24"
                                    height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
    ...
    "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"shrink-0 size-3.5 text-gray-500 dark:text-neutral-500 \" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
                        ...
                        class="hidden" name="mention_id">
                        @php
                            $value = old('mention_id') ?? $parcours->mention_id;
                        @endphp
                        @foreach ($mentions as $mention)
                            <option value="{{ $mention->id }}" {{ $value == $mention->id ? 'selected' : '' }}>
                                {{ $mention->design }}</option>
                        @endforeach
                    </select>
                    <!-- End Select -->
                    @error('mention_id')
